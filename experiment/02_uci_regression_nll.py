...
def one_fold(X, Y, kth, fold_kth):
    ...
    NLL_val_eachitr = [ model.negative_loglikelihood_with_standardised_output(Y_val, P_val, Y_train_std) for P_val in P_val_eachitr ]
    bestitr = int( np.argmin(NLL_val_eachitr) + 1 )
    ...
    NLL_test = model.negative_loglikelihood_with_standardised_output(Y_test, P_test, Y_train_std)
    print("[{:02d}th trial NLL] Test {:.4f} | Val {:.4f} | Bestitr {:d}".format(kth, NLL_test, np.min(NLL_val_eachitr), bestitr))
    return NLL_test, bestitr
...
def main(args):
    ...
    NLL_tests = np.array(results)

    print("=== NLL Test Summary {:.4f} +/- {:.4f} ===".format(np.mean(NLL_tests[:,0]), np.std(NLL_tests[:,0])))
    pd.DataFrame(NLL_tests).to_csv("../result/" + args.id + "_test_nll_" + args.dataset + ".csv", index=False, header=["NLL", "bestitr"])
